...
- **The kit owns the chat UI.** When you embed Diosc, the assistant-kit web
  components are loaded from **DioscHub** at runtime and render the whole chat
...
```ts
import { createDiosc } from '@dioschub/client';
...
| `backendUrl` | `string` | — | DioscHub URL. |
...
3. Your endpoint authenticates the user and forwards the identity to DioscHub's
...
app.post('/api/diosc/bind', requireAuth, async (req, res) => {
  const { wsId } = req.body;
  // Forward the authenticated identity to DioscHub.
...
> See the DioscHub docs for the authoritative bind protocol, including feature
> flags and REST-token rotation.
...
import { createDiosc, type DioscInstance } from '@dioschub/client';
...
```html
<script type="module">
  import { createDiosc } from 'https://esm.sh/@dioschub/client';
...
See the DioscHub docs for the full rationale.
